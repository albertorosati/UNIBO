...
void showList(List l) { // NON PRIMITIVE
	printf("{\n");
	while (!isEmpty(l)) {
		printf("[%s: %s]", l->value.matricola, l->value.corso);
		l = tail(l);
...
int memberList(element el, List l) {
	...
	while (!isEmpty(l) && !result) {
		result = (strcmp(el.matricola, head(l).matricola) == 0);
		if (!result)
			l = tail(l);
	}
	return result;
...
int getSizeByCorso(List l, char * corso)
{
	int result = 0;
	while (!isEmpty(l)) {
		if (strcmp(corso, l->value.corso) == 0) {
			result++;
		}
		l = l->next;
	}
