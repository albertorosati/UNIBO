...
void showList(List l) { // NON PRIMITIVE
	printf("{\n");
	while (!isEmpty(l)) {
		printf("	[nome: %s, squadra: %s, cStart: %d, cA: %d, cB: %d, cC: %d, cEnd: %d]", 
			l->value.nome, l->value.squadra, l->value.controlloStart, l->value.controlloA, 
			l->value.controlloB, l->value.controlloC, l->value.controlloEnd);
		l = tail(l);
...
int memberList(element el, List l) {
	...
	while (!isEmpty(l) && !result) {
		result = (strcmp(el.squadra, head(l).squadra) == 0);
		if (!result)
			l = tail(l);
	}
	return result;
